...
from scripts.hook import CrossAttentionHook
...
def main():
    ...
    attention_path = os.path.join(outpath, "attention")
    ...
    import json
    # Iterate through each JSON file in the bbox_info directory
    for json_file in os.listdir(bbox_info_dir):
        if not json_file.endswith('.json'):
            continue  # Skip non-JSON files

        # Load JSON data
        json_path = os.path.join(bbox_info_dir, json_file)
        with open(json_path, 'r') as f:
            item = json.load(f)

        # Extract paths from the JSON data
        image_path = item[0]["image_path"]
        mask_path = item[0]["mask_path"]
        reference_path = item[0]["reference_image_path"]
        filename = os.path.basename(image_path)
        
        # Check if the generated files already exist
        output_file = os.path.join(result_path, f"{filename[:-4]}_{str(opt.seed)}.png")

        # If any of the output files exist, skip processing for this image
        ...
        with torch.no_grad():
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    filename=os.path.basename(image_path)
                    img_p = Image.open(image_path).convert("RGB").resize(resize_size)
                    ...
                    mask=Image.open(mask_path).convert("L").resize(resize_size)
                    ...
                    """ For Debugging"""
                    hook = CrossAttentionHook()
                    hook.take_module(sampler.model.model)
                    ...
                    """For Debugging"""
                    input_image = torch.clamp(x_samples_ddim, min=-1.0, max=1.0).cpu()
                    ...
                    """ For Debugging """
                    save_dir = os.path.join(attention_path)
                    attention_maps = hook.make_attention_maps()
                    example_img_idx = json_file.split('_')[2].split('.')[0]
                    if hook.per_time_step:
                        for idx, key in enumerate(attention_maps.keys(),1):
                            hook.make_images(input_image, cloth, attention_maps[key], save_dir, f"{example_img_idx}-time-{idx}") 
                    elif hook.per_attention_head:
                        for idx, attention_map in enumerate(attention_maps,1):
                            hook.make_images(input_image, cloth, attention_map, save_dir, f"{example_img_idx}-head-{idx}")
                    else:
